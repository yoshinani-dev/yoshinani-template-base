        ...
        {payload
          ...
          .map((item, index) => {
            ...
            if (
              ...
              const fillValue = (item.payload as { fill?: unknown }).fill
              payloadFill =
                typeof fillValue === "string" ? fillValue : undefined
            }
  ...
  return (
    ...
    >
      {payload
        .filter((item) => item.type !== "none")
        .map((item) => {
          const dataKey = item.dataKey ? String(item.dataKey) : "value"
          const key = `${nameKey || dataKey}`
          const itemConfig = getPayloadConfigFromPayload(config, item, key)

          return (
            <div
              key={String(item.value ?? dataKey)}
              className={cn(
